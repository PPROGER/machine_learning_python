            ...
            while True:
                ...
                try:
                    ...
                    if((statement.find("найти")!=-1) or (statement.find("поиск")!=-1) or (statement.find("найди")!=-1) or (statement.find("дайте")!=-1) or (statement.find("mighty")!=-1)):
                        ...
                        self.openurl('https://www.google.com/search?client=firefox-b-d&q=' + statement, "Я нашла следующие результаты")
                    ...
                    if((statement.find("смотреть")!=-1) and ((statement.find("фильм")!=-1) or (statement.find("film")!=-1))):
                        ...
                        statement=statement.strip()
                        self.openurl('https://www.google.com/search?client=firefox-b-d&q=Смотреть+онлайн+фильм+' + statement, "Выберите сайт где смотреть фильм")
